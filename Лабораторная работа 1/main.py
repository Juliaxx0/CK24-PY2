...
class Student:
    def __init__(self, name: str, direction: str, course: int, list_of_courses: list):
        """
        Создание и подготовка к работе объекта "Студент"

        :param name: Имя студента
        :param direction: Направление на котором учится
        :param course: Номер курса
        :param list_of_courses: Список курсов

        Примеры:
        >>> student = Student("Name", "Math", 2, ["Mathematics", "Geometry", "Chemistry"])
        """
        if not isinstance(name, str):
            raise TypeError("Имя должно быть типа str")
        if not name.isalpha():
        ...
        if not direction.isalpha():
            raise ValueError("В названии направления должны быть только буквы")
